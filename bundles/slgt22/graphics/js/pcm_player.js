class PCMPlayer {
  ...
  initAudioContext() {
    // PATCH, allow passing in an audio context
    if (this.option.audioCtx) {
      this.audioCtx = this.option.audioCtx;
    } else {
      // 初始化音频上下文的东西
    ...
    // PATCH allow connecting to different node
    if (this.option.destination) {
      this.gainNode.connect(this.option.destination);
    } else {
